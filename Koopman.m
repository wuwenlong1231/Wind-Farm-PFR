...
for j=1:b%此处为了C的反归一化
    for i=1:H
       ...
       input1(3,(j-1)*H+i)=Kf1(i,j);%控制量
       input1(4,(j-1)*H+i)=windspeed1(i,j);
       output1(1,(j-1)*H+i)=wspeed1(i+1,j);%状态输出量
       output1(2,(j-1)*H+i)=frequency(i+1,j);
       output1(3,(j-1)*H+i)=Kf1(i+1,j);
       output1(4,(j-1)*H+i)=windspeed1(i+1,j);
    end
end
...
%%
xlswrite('D:\本科期间各种材料\近期论文材料\PES会议材料\Koopman数据10.15更新.xlsx',M1,'风机1频率下降M');
xlswrite('D:\本科期间各种材料\近期论文材料\PES会议材料\Koopman数据10.15更新.xlsx',C1,'风机1频率下降C');
