function [output1]=Time_modeldown(Kfi1,Kfi2,X2,f,I)%函数
N=20;
for i=1:N/2
   Kfi(1,i)=Kfi1;
   Kfi(1,i+N/2)=Kfi2;
...
Pm(i,j)=0.5*1.3*S*Cp(i,j)*windspeed(i,j)^3;%由转速和风速决定的
    ...
    %风场
     Pef(i,1)=K1*(frequency(1,1)-frequency(i,1))*1000000;%火电厂增加出力（下垂＋惯性）
...
%      风机
    ...
    for j=1:N
    SOE(i,j)=(wspeed(i,j)^2-0.6^2)/(1.3^2-0.6^2);
    end
    for j=1:N
    ...
    for j=1:N
        if wspeed(i+1,j)>1.35
               wspeed(i+1,j)=1.6;
     elseif wspeed(i+1,j)<0.6
             wspeed(i+1,j)=0.5;
        end
    end  
end
